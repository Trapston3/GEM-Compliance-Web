...
export default async function TenderMatrixPage({ params }: { params: Promise<{ tenderId: string }> }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <Header title="Compliance Matrix" currentUser={{ nameEn: dbUser.nameEn, role: dbUser.role }} tender={{ id: tender.id, name: tender.name }} />
      <Dashboard view="matrix" tender={tender} bidders={withStatuses} checklistItems={items} currentUser={{ id: String(dbUser.id), nameHi: dbUser.nameHi, nameEn: dbUser.nameEn, phone: dbUser.phone, email: dbUser.email, role: dbUser.role }} />
    </div>
  );
}
